...
function getElementIndex (givennumber) {
    for (let index = 0; index < numbers.length; index++)  {
        if (numbers.includes(givennumber)){
          return numbers.indexOf(givennumber);
        } else {
 ...
 */
 ...
 function lowestNumber() {
    let lowNumber = forLowestNumber[0];
    for (let i = 0; i < forLowestNumber.length; i++) {
      if (lowNumber > forLowestNumber[i]) {
        lowNumber = forLowestNumber[i];
      }
    }
    return lowNumber;
  }